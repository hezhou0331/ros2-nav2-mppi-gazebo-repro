...
1. 单颗 A2 头部 3D 雷达输出 `/lidar/points`，投影为 `/scan`。投影只使用
   `-0.50` 至 `0.12 m` 高度带，并丢弃距雷达小于 `0.40 m` 的近场回波。
   局部/全局代价地图和碰撞监控使用同一条 `/scan` 派生数据；脚本用 A2
   模型中的雷达前移 `0.33767 m` 外参把每个端点换算到 `base_link`，只将
   落在 `0.60 m` 仿真机体包络内部的回波替换为无回波，包络外的平地障碍
   仍保留。
...
4. 脚本保存 `maps/atec_practice_world.{pgm,yaml}`，停止建图进程组，以同一地图
   重启 Gazebo、仿真恒等 `map -> a2/odom`、AMCL 诊断节点和 Nav2。该恒等 TF
   只适用于地图直接建立在 Gazebo 无漂移里程计坐标中的演示；实机必须删除它并
   由经验证的 AMCL/LIO 发布。保存后只把机器人实际巡航折线周围 `0.65 m`
...
5. 任务节点通过 `/navigate_to_pose` 依次执行 `map(-2.5,-0.5,0)` 与
   ...
   的最终距离不超过 `0.35 m`。
6. Regulated Pure Pursuit 在 `10 Hz` 下以 `0.15 m/s` 跟踪 NavFn 路径，启用
   曲率调速、到点减速、原地朝向调整和 `1.0 s` 前向碰撞检查。位姿进度检查
   要求 `20 s` 内移动 `0.05 m` 或旋转 `0.20 rad`。
...
- 控制器周期性停滞：对比 `/scan` 与 `/collision_scan`。局部、全局代价地图
  和碰撞监控都应使用后者；它按雷达外参将端点换算到 `base_link` 后，只能
  移除 `0.60 m` 足迹内部的点，不能用高度阈值或以雷达为圆心的简单量程
  阈值掩盖平地障碍。
...
实测雷达外参、按 URDF/3D 几何实现的机体自过滤、P7 安装板负载与实机低速
验收，才能进入实际比赛测试。仿真的 `0.60 m` 整足迹回波过滤和 `2.0 s`
传感健康窗口不得直接用于真实机器人。
