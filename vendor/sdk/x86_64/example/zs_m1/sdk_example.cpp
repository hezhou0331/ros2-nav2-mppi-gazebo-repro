// ZS_M1 基础连通性示例
//
// 功能：通过 UDP 连接到 ZS_M1 机器人，以固定频率（默认 500Hz）向所有执行器（不含轮）发送
// 零力矩阻尼指令（kd=5），同时打印机器人上报的状态数据。每隔 5 秒切换一次补光灯。
// 运行结束后打印收到的状态帧总数，可用于验证 SDK 通信链路是否正常。
//
// 典型用法：
//   ./lowlevel_sdk_example                         # 使用默认参数
//   ./lowlevel_sdk_example --host 192.168.1.100    # 指定机器人 IP
//   ./lowlevel_sdk_example --max-cycles 5000 --period-ms 4  # 250Hz 运行 20s
...
using robot_sdk_lowlevel::zs_m1::kActuatorCount;
using robot_sdk_lowlevel::zs_m1::LowLevelCommand;
using robot_sdk_lowlevel::zs_m1::LowLevelState;
using robot_sdk_lowlevel::zs_m1::ZsM1Client;
...
int main(int argc, char** argv) {
  std::printf("[INFO] SDK example starting...\n");
  ...
  std::printf("[INFO] ZsM1Client initialized successfully.\n");
  ...
  client.RegisterLowLevelDataRecvCallback(
      [&cnt, &config, &recv_affinity_once](std::shared_ptr<LowLevelState> state) {
        ...
        std::printf("%s\n", robot_sdk_lowlevel::zs_m1::ToString(*state).c_str());
        ++cnt;
      });
  ...
  std::printf("[INFO] SDK example finished.\n");
